 ...
 */
@Service("findStudentForTakeBookBySpotCodeDAO")
public class FindStudentForTakeBookBySpotCodeDAOImpl extends BaseQueryDao
    ...
    public List<Object[]> findListByWhere(Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        sql.append("select DISTINCT l.name levelName, sp.name specName, s.code, s.name, s.study_enter_year, case when s.study_quarter = 0 then '春' else '秋' end quarter, s.mobile, se.buy ");
        sql.append("from student_book_order sbo, sync_student s, sync_level l, sync_spec sp, student_expense se ");
        sql.append("where sbo.student_code = s.code and s.code = se.student_code and s.level_code = l.code and s.spec_code = sp.code and sbo.semester_id = se.semester_id ");
