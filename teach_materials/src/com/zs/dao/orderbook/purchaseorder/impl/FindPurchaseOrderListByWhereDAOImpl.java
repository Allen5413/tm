 ...
 */
@Service("findPurchaseOrderListByWhereDAO")
public class FindPurchaseOrderListByWhereDAOImpl extends BaseQueryDao
    ...
    public List<Object[]> findListByWhere(Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        sbSql.append("select po.code, ic.id, ic.name as channelName, tmt.name as tmTypeName, sum(potm.teach_material_count) as tmCount, sum(potm.teach_material_count * tm.price) as price, sum(put_storage_count) as put_count, po.creator, po.create_time from ");
        ...
        sbSql.append("group by po.code, ic.name, tmt.name, po.creator, po.create_time ");
        if(null != sortMap) {
