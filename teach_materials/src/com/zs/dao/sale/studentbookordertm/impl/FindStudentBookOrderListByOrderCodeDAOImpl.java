...
public class FindStudentBookOrderListByOrderCodeDAOImpl extends BaseQueryDao
    ...
    public List findListByWhere(Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        sql.append("(CASE WHEN sbo.state < 2 THEN tm.state = 0 WHEN sbo.state >= 4 THEN sbotm.is_send = 1 ELSE 1=1 END) and sbo.order_code = sbotm.order_code and sbotm.teach_material_id = tm.id and sbotm.course_code = sc.code and sbo.order_code = ? ");
