 ...
 */
@Service("findStudentBookOrderTMPrintByOrderIdDAO")
public class FindStudentBookOrderTMPrintByOrderIdDAOImpl extends BaseQueryDao
    ...
    public List findListByWhere(Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        sql.append("FROM semester ss, sync_spot spot, sync_student s, sync_spec sp, sync_level l, student_book_order sbo, student_book_order_tm sbotm, teach_material tm, sync_course c ");
        sql.append("WHERE ss.id = sbo.semester_id AND s.spot_code = spot.code AND s.code = sbo.student_code AND s.spec_code = sp.code AND s.level_code = l.code AND c.code = sbotm.course_code AND sbo.order_code = sbotm.order_code AND sbotm.teach_material_id = tm.id AND sbotm.is_send = 1 AND sbo.is_stock = 0 AND sbotm.count > 0 ");
