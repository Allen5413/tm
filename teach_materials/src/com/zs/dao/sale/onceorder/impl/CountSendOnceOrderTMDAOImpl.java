 ...
 */
@Service("countSendOnceOrderTMDAO")
public class CountSendOnceOrderTMDAOImpl extends BaseQueryDao
    ...
    public PageInfo findPageByWhere(PageInfo pageInfo, Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        StringBuilder sql = new StringBuilder("from teach_material tm, student_book_once_order sbo,  student_book_once_order_tm sbotm, sync_student s, student_book_order_package sbop ");
        sql.append("where tm.id = sbotm.teach_material_id and sbo.id = sbotm.order_id and sbo.package_id = sbop.id and sbo.student_code = s.code and sbotm.is_send = 1 and sbo.state > 4 and sbotm.count > 0 ");
