 ...
 */
@Service("findOnceOrderForSpotCountDAO")
public class FindOnceOrderForSpotCountDAOImpl extends BaseQueryDao
    ...
    public PageInfo findPageByWhere(PageInfo pageInfo, Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        sql.append("from sync_spot sp, sync_student s, student_book_once_order sbo, student_book_once_order_tm sbotm, teach_material tm ");
        sql.append("where sp.`code` = s.spot_code AND s.`code` = sbo.student_code AND sbo.id = sbotm.order_id AND sbotm.teach_material_id = tm.id AND tm.state = 0 AND sbotm.count > 0 and sbo.is_send_student = 0 ");
