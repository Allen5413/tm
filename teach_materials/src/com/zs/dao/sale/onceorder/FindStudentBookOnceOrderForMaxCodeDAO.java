...
public interface FindStudentBookOnceOrderForMaxCodeDAO extends EntityJpaDao<StudentBookOnceOrder, Long> {
    @Query(nativeQuery = true, value = "select t.* from (select * from student_book_once_order where semester_id = ?1 and RIGHT(order_code,6) < 900000 order by RIGHT(order_code,6) desc limit 1) t")
