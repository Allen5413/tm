 ...
 */
@Service("countSendStudentBookOrderByTMIdDAO")
public class CountSendStudentBookOrderByTMIdDAOImpl extends BaseQueryDao
    ...
    public List<Object[]> findListByWhere(Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        sql.append("FROM student_book_order sbo, student_book_order_tm sbotm, sync_student s, sync_spot sp, student_book_order_package sbop ");
        sql.append("WHERE sbo.order_code = sbotm.order_code AND sbo.package_id = sbop.id AND sbo.student_code = s.code AND sp.code = s.spot_code AND sbo.state > 3 AND sbotm.count > 0 ");
