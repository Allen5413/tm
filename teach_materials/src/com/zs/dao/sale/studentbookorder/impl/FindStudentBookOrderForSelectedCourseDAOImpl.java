...
public class FindStudentBookOrderForSelectedCourseDAOImpl extends BaseQueryDao
    ...
    public List findListByWhere(Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        sql.append(") ");
        sql.append(" UNION ");
        sql.append("SELECT ir.issue_channel_id, s.code, sbotm.course_code, sbo.order_code ");
        sql.append("FROM student_book_order sbo ");
        sql.append(" INNER JOIN sync_student s ON s.code = sbo.student_code ");
        sql.append(" INNER JOIN issue_range ir ON s.spot_code = ir.spot_code ");
        sql.append("INNER JOIN student_book_order_tm sbotm ON sbo.order_code = sbotm.order_code ");
        ...
        if(!StringUtils.isEmpty(courseCode)){
            sql.append("AND sbotm.course_code = ? ");
            params.add(courseCode);
        }
        if(!StringUtils.isEmpty(tmId)){
            sql.append("AND sbotm.teach_material_id != ? ");
