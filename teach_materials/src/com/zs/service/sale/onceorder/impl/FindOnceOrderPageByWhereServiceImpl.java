 ...
 */
@Service("findOnceOrderPageByWhereService")
public class FindOnceOrderPageByWhereServiceImpl extends EntityServiceImpl implements FindOnceOrderPageByWhereService {

    @Resource
    private FindPageByWhereDAO findOnceOrderPageByWhereDAO;
    ...
    public PageInfo findPageByWhere(PageInfo pageInfo, Map<String, String> map, Map<String, Boolean> sortMap) throws Exception {
        pageInfo = findOnceOrderPageByWhereDAO.findPageByWhere(pageInfo, map, sortMap);
        if(null != pageInfo && null != pageInfo.getPageResults() && 0 < pageInfo.getPageResults().size()){
            ...
            for(Object[] objs : list){
                ...
                studentBookOnceOrder.setOperateTime((Date) objs[11]);
                JsonConfig jsonConfig = new JsonConfig();
                jsonConfig.registerJsonValueProcessor(Date.class, new DateJsonValueProcessorTools());
                JSONObject jsonObject = JSONObject.fromObject(studentBookOnceOrder, jsonConfig);
                ...
                jsonObject.put("levelName", LevelEnum.getDescn(objs[6].toString()));
