 ...
 */
@Service("findStudentBookOrderTMForSpotPrintService")
public class FindStudentBookOrderTMForSpotPrintServiceImpl extends EntityServiceImpl
        implements FindStudentBookOrderTMForSpotPrintService {

    @Resource
    private FindListByWhereDAO findStudentBookOrderTMForSpotPrintDAO;
    @Resource
    ...
    public JSONObject findStudentBookOrderTMForSpotPrint(HttpServletRequest request, Map<String, String> paramsMap, Map<String, Boolean> sortMap)throws Exception {
        List<Object[]> resultList = findStudentBookOrderTMForSpotPrintDAO.findListByWhere(paramsMap, sortMap);
            ...
            for(Object[] objs : resultList){
                ...
                if(i == 0 || !beforeOrderCode.equals(orderCode)){
                    ...
                    if(state == StudentBookOrder.STATE_CONFIRMED && null != request) {
                        //修改订单状态为分拣中
                        editStudentBookOrderForStateService.editStudentBookOrderForState(orderCode, null, StudentBookOrder.STATE_SORTING, operateTime, UserTools.getLoginUserForName(request));
                    }
                    if(0 < i){
