 ...
 */
@Service("findStudentBookOrderTMForStudentPrintService")
public class FindStudentBookOrderTMForStudentPrintServiceImpl extends EntityServiceImpl
        implements FindStudentBookOrderTMForStudentPrintService {

    @Resource
    private FindStudentBookOrderTMForStudentPrintDAO findStudentBookOrderTMForStudentPrintDAO;
    @Resource
    ...
    public JSONObject print(long semesterId, int state, int pageNum, int countNum, String operateTime, String spotCode, HttpServletRequest request)throws Exception {
        ...
        if(state == StudentBookOrder.STATE_CONFIRMED) {
            if(StringUtils.isEmpty(spotCode)) {
                resultList = findStudentBookOrderTMForStudentPrintDAO.find(semesterId, state, pageNum * countNum, countNum);
            }else{
                resultList = findStudentBookOrderTMForStudentPrintDAO.find(semesterId, state, pageNum * countNum, countNum, spotCode);
            }
        }else{
            if(StringUtils.isEmpty(spotCode)) {
                resultList = findStudentBookOrderTMForStudentPrintDAO.findPrint(semesterId, state, operateTime);
            }else{
                resultList = findStudentBookOrderTMForStudentPrintDAO.findPrint(semesterId, state, operateTime, spotCode);
            }
        }
        ...
        if(null != resultList && 0 < resultList.size()){
            ...
            for(Object[] objs : resultList){
                ...
                if(i == 0 || !beforeOrderCode.equals(orderCode)){
                    ...
                    if(state == StudentBookOrder.STATE_CONFIRMED && null != request) {
                        //修改订单状态为分拣中
                        editStudentBookOrderForStateService.editStudentBookOrderForState(orderCode, null, StudentBookOrder.STATE_SORTING, nowOperateTime, UserTools.getLoginUserForName(request));
                    }
                    if(0 < i){
