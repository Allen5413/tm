 ...
 */
@Service("findStudentBookOrderForSpotCountService")
public class FindStudentBookOrderForSpotCountServiceImpl extends EntityServiceImpl implements FindStudentBookOrderPageByWhereService {

    @Resource
    private FindPageByWhereDAO findStudentBookOrderForSpotCountDAO;

    @Override
    @Transactional
    public PageInfo findPageByWhere(PageInfo pageInfo, Map<String, String> map, Map<String, Boolean> sortMap) throws Exception {
        pageInfo = findStudentBookOrderForSpotCountDAO.findPageByWhere(pageInfo, map, sortMap);
        if(null != pageInfo && null != pageInfo.getPageResults() && 0 < pageInfo.getPageResults().size()){
            ...
            for(Object[] objs : list){
                ...
                if(5 < objs.length) {
                    ...
                    studentBookOrder.setOperateTime((Date) objs[5]);
