...
import com.zs.dao.sale.onceorder.FindOnceOrderForMaxPrintSortDAO;
 ...
 */
@Service("findOnceOrderTMPrintByOrderIdService")
public class FindOnceOrderTMPrintByOrderIdServiceImpl extends EntityServiceImpl
        implements FindOnceOrderTMPrintByOrderIdService {

    @Resource
    private FindListByWhereDAO findOnceOrderTMPrintByOrderIdDAO;
    @Resource
    private FindOnceOrderForMaxPrintSortDAO findOnceOrderForMaxPrintSortDAO;
    @Resource
    ...
    public JSONObject find(String... ids)throws Exception {
        ...
        paramsMap.put("state", StudentBookOnceOrder.STATE_SEND+"");
        List<Object[]> resultList = findOnceOrderTMPrintByOrderIdDAO.findListByWhere(paramsMap, null);
    ...
    public void editPrintSort(String... ids) throws Exception {
        ...
        int printSort = 0;
        for(String id : ids){
            StudentBookOnceOrder studentBookOnceOrder = findOnceOrderForMaxPrintSortDAO.get(Long.parseLong(id));
            if(null != studentBookOnceOrder && null == studentBookOnceOrder.getPrintSort() && studentBookOnceOrder.getState() >= StudentBookOnceOrder.STATE_SEND) {
                Student student = findStudentByCodeDAO.getStudentByCode(studentBookOnceOrder.getStudentCode());
                //查询当前最大的顺序号
                Integer maxSort = findOnceOrderForMaxPrintSortDAO.find(studentBookOnceOrder.getSemesterId(), student.getSpotCode());
                if(null == maxSort){
                    maxSort = 0;
                }
                printSort++;
                studentBookOnceOrder.setPrintSort(maxSort + printSort);
                findOnceOrderForMaxPrintSortDAO.update(studentBookOnceOrder);
            }
        }
    }
}
