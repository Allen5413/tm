...
public class AddSpotEBuyServiceImpl extends EntityServiceImpl<SpotExpense,SpotExpenseDao> implements AddSpotEBuyService {
    ...
    public void addSpotEBuy(SpotExpenseBuy spotExpenseBuy,String userName) throws Exception {
        ...
        if(null == spotExpenseList || 1 > spotExpenseList.size()){
            ...
            spotExpense.setBuy(new BigDecimal(spotExpenseBuy.getMoney()).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
            spotExpense.setPay(0d);
            ...
            for (SpotExpense spotExpense : spotExpenseList) {
                double pay = null == spotExpense.getPay() ? 0 : spotExpense.getPay();    //交的钱
                double buy = null == spotExpense.getBuy() ? 0 : spotExpense.getBuy();    //消费的钱
                ...
                if (semester.getId() == spotExpense.getSemesterId()) {
                    isNowSemester = true;
                    spotExpense.setPay(new BigDecimal(moreMoney).add(new BigDecimal(pay)).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
                    spotExpense.setBuy(new BigDecimal(buy).add(new BigDecimal(tempMoney)).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
                    ...
                    double temp = new BigDecimal(pay).subtract(new BigDecimal(actualBuy)).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
                    ...
                    if(temp > 0){
                        ...
                        spotExpense.setPay(new BigDecimal(spotExpense.getPay()).subtract(new BigDecimal(temp)).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
                        findSpotRecordBySpotCodeDao.update(spotExpense);
                    }
                }
            }
            if (!isNowSemester) {
                ...
                SpotExpense spotExpense = new SpotExpense();
                spotExpense.setSpotCode(spotCode);
                spotExpense.setPay(new BigDecimal(moreMoney).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
                spotExpense.setBuy(new BigDecimal(spotExpenseBuy.getMoney()).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
