    ...
    <%--}else{--%>
      <%--if(isVaild){--%>
        <%--$.ajax({--%>
          ...
          <%--data:{"codeMoneys" : codeMoneys.substr(0, codeMoneys.length-1), "spotCode":$("#spotCode").val()},--%>
          <%--async: false,--%>
...
<form id="stuForm" name="stuForm" action="${pageContext.request.contextPath}/addInvoice/findStudent.htm" method="post">
  <table width="100%">
    <tr height="40">
      ...
      <td>
        <select id="add_spotCode" name="add_spotCode" >
          <option value="">--请选择--</option>
          <c:forEach var="spot" items="${spotList}">
            <option value="${spot.code}" <c:if test="${param.add_spotCode eq spot.code}">selected="selected" </c:if> >[${spot.code}]${spot.name}</option>
          </c:forEach>
        </select>
      </td>
