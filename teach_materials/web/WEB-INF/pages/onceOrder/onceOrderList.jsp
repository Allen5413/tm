...
<c:if test="${method eq 'search'}">
  <table class="am-table am-table-bordered am-table-striped am-table-hover" style="width:100%;">
    ...
    <c:forEach var="json" items="${pageInfo.pageResults}" varStatus="status">
      <tr>
        <td>
          <c:if test="${json.state >= 5 && (json.studentSign == 1 || empty json.studentSign)}">
        ...
        <td>
          <c:choose>
            ...
            <c:when test="${json.state == 2}">采购中</c:when>
          ...
          <div class="doc-dropdown-justify-js">
            <div class="am-dropdown" data-am-dropdown="" id="operateBtn_${status.index+1}">
              ...
              <ul class="am-dropdown-content">
                ...
                <c:if test="${json.state >= 1 && json.state <= 3}">
                ...
                <c:if test="${0 < json.state && json.state < 5}">
...
<script>
  ...
  function isSendStudent(studentCode, orderId){
    app.openDialog("${pageContext.request.contextPath}/editOnceOrderForIsSendStudent/open.htm?studentCode="+studentCode+"&orderId="+orderId, "修改订单是否邮寄给学生", 600, 0.6, function(index){
