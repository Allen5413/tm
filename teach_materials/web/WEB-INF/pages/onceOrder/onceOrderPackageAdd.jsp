...
<form id="addForm" name="addForm" target="_blank" action="${pageContext.request.contextPath}/addOnceOrderPackage/add.htm" method="post">
  <label >学习中心：</label>
  ...
  <button class="am-btn am-btn-primary am-btn-s" type="button" onClick="mergePackage()"><span class="am-icon-refresh"></span> 单个打包</button>
...
<script>
    ...
    if(isSendStudent == 1){
      ...
      setTimeout(function(){
        ...
        $.ajax({
          cache: true,
          type: "POST",
          url:"${pageContext.request.contextPath}/addOnceOrderPackage/searchOrderStudent.htm",
          data:{
            "spotCode":spotCode,
            "orderCode":orderCode,
  ...
  function addPackage(){
    ...
    $.ajax({
      cache: true,
      type: "POST",
      url:"${pageContext.request.contextPath}/addOnceOrderPackage/add.htm",
  ...
  function mergePackage(){
    var url = "${pageContext.request.contextPath}/addOnceOrderPackage/toMergeOpen.htm";
    var falg;
    if(0 <= url.indexOf("?")) {
