    ...
    <div id="spot" class="am-in">
      <div class="am-g am-margin-top">
        ...
        <div class="am-u-sm-11">
          <select id="spotCode" name="spotCode" data-am-selected="{maxHeight: 500, searchBox: 1}" onchange="changeSpot(this)">
            <option value="">--请选择--</option>
            <c:forEach var="spot" items="${spotList}">
              <option value="${spot.code}">[${spot.code}]${spot.name}</option>
            </c:forEach>
          </select>
        </div>
      </div>
      ...
      <div class="am-g am-margin-top">
        ...
        <div class="am-u-sm-11"><input type="text" id="address" name="address" style="width: 600px;" />&nbsp;&nbsp;&nbsp;&nbsp;*必填</div>
      </div>

      <div class="am-g am-margin-top">
        <div class="am-u-sm-1 am-text-right"><label >收件人：</label></div>
        <div class="am-u-sm-11"><input type="text" id="adminName" name="adminName" style="width: 600px;" />&nbsp;&nbsp;&nbsp;&nbsp;*必填</div>
      </div>

      <div class="am-g am-margin-top">
        <div class="am-u-sm-1 am-text-right"><label >手机：</label></div>
        <div class="am-u-sm-11"><input type="text" id="phone" name="phone" style="width: 600px;" />&nbsp;&nbsp;&nbsp;&nbsp;*必填</div>
      </div>

      <div class="am-g am-margin-top">
        <div class="am-u-sm-1 am-text-right"><label >座机：</label></div>
        <div class="am-u-sm-11"><input type="text" id="tel" name="tel" style="width: 600px;" /></div>
      </div>

      <div class="am-g am-margin-top">
        <div class="am-u-sm-1 am-text-right"><label >邮编：</label></div>
        <div class="am-u-sm-11"><input type="text" id="postalCode" name="postalCode" style="width: 600px;" /></div>
      </div>
    </div>
  </div>
  ...
  function changeSpot(obj){
    ...
    if(spotCode == ""){
      ...
      $.ajax({
        cache: true,
        type: "POST",
        url:"${pageContext.request.contextPath}/findSpotByCode/find.htm",
        data:{code:spotCode},
        async: false,
        success: function(data) {
          if(data.state == 0){
            $("#address").val(data.spot.address);
            $("#adminName").val(data.spot.adminName);
            $("#phone").val(data.spot.phone);
            $("#tel").val(data.spot.tel);
            $("#postalCode").val(data.spot.postalCode);
          }else{
            app.msg(data.msg, 1);
          }
        }
