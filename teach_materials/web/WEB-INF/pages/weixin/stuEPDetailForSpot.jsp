...
<c:if test="${empty orderList}">
  <center><div style="color:#ff6c0f;">暂时没有未确认的订单信息</div></center>
</c:if>
<c:if test="${!empty orderList}">
  <section class="bg-f4-pt pm-top-40">
    <div class="auto w order-info-list">
      <c:forEach var="json" items="${orderList}">
        <div class="mod-order" onclick="searchDetail('${json.flag}', '${json.orderCode}')">
...
<script type="text/javascript">
  ...
  function searchDetail(flag, orderCode){
    if(0 == flag || "0" == flag){
      location.href = "${pageContext.request.contextPath}/wxSearch/orderDetail.htm?code="+orderCode+"&isSpotUser=1&studentCode=${param.code}";
    }
    if(1 == flag || "1" == flag){
      location.href = "${pageContext.request.contextPath}/wxSearch/onceOrderDetail.htm?code="+orderCode+"&isSpotUser=1&studentCode=${param.code}";
    }
  }
</script>
