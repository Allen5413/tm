  ...
  <%--<div class="main-content">--%>
    <%--<ul class="create_info_list">--%>
      <%--<li>--%>
        ...
        <%--<select class="select" name="menuId">--%>
          ...
          <%--<c:forEach var="menu" items="${menuList}">--%>
            <%--<option value="${menu.id}" <c:if test="${resource.menuId == menu.id}">selected="selected"</c:if>>--%>
              <%--${menu.name}--%>
            <%--</option>--%>
          <%--</c:forEach>--%>
        <%--</select>--%>
      ...
      <%--<li>--%>
        ...
        <%--<input type="text" class="input_240" name="name" value="${resource.name}" maxlength="50" />--%>
      <%--</li>--%>
      <%--<li>--%>
        <%--<label class="lab_80">url：</label>--%>
        <%--<input type="text" class="input_240" name="url" value="${resource.url}" maxlength="200" />--%>
      <%--</li>--%>
      <%--<li>--%>
        <%--<label class="lab_80 left">备注：</label>--%>
        <%--<textarea class="textArea left" name="remark" maxlength="500">${resource.remark}</textarea>--%>
        <%--<div class="clear"></div>--%>
      <%--</li>--%>
      <%--<li>--%>
  ...
  <%--function sub(){--%>
    <%--if($("#editForm").valid()){--%>
      <%--$.ajax({--%>
        <%--cache: true,--%>
        <%--type: "POST",--%>
        <%--url:"/editResource/resourceEdit.htm",--%>
...
<form class="am-form" id="editForm" method="post">
  ...
  <div class="am-g am-margin-top">
    ...
    <div class="am-u-sm-4">
      <select class="select" name="menuId" id="edit_menuId">
        <option value="">--请选择--</option>
        <c:forEach var="menu" items="${menuList}">
          <option value="${menu.id}" <c:if test="${resource.menuId == menu.id}">selected="selected"</c:if>>
            ${menu.name}
  ...
  <div class="am-g am-margin-top">
    ...
    <div class="am-u-sm-4">
      <input class="am-input-sm" type="text" placeholder="输入名称" required id="edit_name" name="name" value="${resource.name}" />
    </div>
